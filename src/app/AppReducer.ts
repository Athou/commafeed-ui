...
const entriesReducer: Reducer<EntriesState, Actions> = (
  state = { loading: false },
  action
) => {
  switch (action.type) {
    ...
    case "entries.addEntries":
      ...
      const entriesToAdd = action.entries.filter(
        e => !lodash.some(state.entries, { id: e.id })
      );
      return {
        ...state,
        entries: state.entries && state.entries.concat(entriesToAdd),
        hasMore: action.hasMore
      };
    ...
    case "entries.setRead":
      const newEntries = state.entries
        ? state.entries.map(e =>
            e.id === action.id ? Object.assign({}, e, { read: action.read }) : e
          )
        : undefined;
...
const settingsReducer: Reducer<ISettings | null, Actions> = (
  state = null,
  action: Actions
) => {
  switch (action.type) {
...
const redirectReducer: Reducer<RedirectState, Actions> = (
  state = {},
  action
) => {
  switch (action.type) {
    ...
    case "navigateToCategory":
      return {
        ...state,
        redirectTo: Routes.app.category.create({
    ...
    case "navigateToFeed":
      return {
        ...state,
        redirectTo: Routes.app.feed.create({ feedId: String(action.feedId) })
      };
  ...
  tree: {
    ...
    toggleCategoryExpanded(
      ...
      return (dispatch, getState) => {
        ...
        const category = flattenCategoryTree(state.tree.root).find(
          c => +c.id === categoryId
        );
        if (!category) return;
  ...
  entries: {
    ...
    reload(): ThunkAction<void, State, void, Actions> {
      return (dispatch, getState) => {
        ...
        fetchEntries(
          ...
          .finally(() =>
            dispatch({ type: "entries.setLoading", loading: false })
          );
      };
    },
    ...
    selectEntry(entry: Entry): ThunkAction<void, State, void, Actions> {
      return (dispatch, getState) => {
        const state = getState();
        const wasExpanded =
          entry.id === state.entries.selectedEntryId
            ? state.entries.selectedEntryExpanded
            : false;
    ...
    selectNextEntry(): ThunkAction<void, State, void, Actions> {
      return (dispatch, getState) => {
        ...
        const currentEntry = entries.find(
          e => e.id === state.entries.selectedEntryId
        );
    ...
    markAllAsRead(
      ...
      return dispatch => {
        ...
        service
          .mark(new MarkRequest({ id, olderThan, read: true }))
          .then(() => {
...
function fetchEntries(
  ...
  switch (source) {
    case "category":
      return clients.category.entries(
        id,
        readingMode,
        undefined,
        offset,
