...
export const Navbar: React.FC = () => {
  ...
  const readingMode = useSelector(
    (state: State) => state.settings && state.settings.readingMode
  );
  const readingOrder = useSelector(
    (state: State) => state.settings && state.settings.readingOrder
  );
  ...
  function markAllClicked() {
    if (!id || !source) return;
    dispatch(
      ActionCreator.entries.markAllAsRead(id, source, new Date().getTime())
    );
  }
  ...
  function readingModeClicked() {
    ...
    const mode: ReadingMode =
      readingMode === ReadingMode.All ? ReadingMode.Unread : ReadingMode.All;
  ...
  function readingOrderClicked() {
    ...
    const order: ReadingOrder =
      readingOrder === ReadingOrder.Desc ? ReadingOrder.Asc : ReadingOrder.Desc;
      ...
      <Toolbar variant="dense">
        ...
        <div>
          <IconButton
            color="inherit"
            onClick={() =>
              dispatch(ActionCreator.redirect.navigateToSubscribe())
            }
          >
            <Add />
          </IconButton>
